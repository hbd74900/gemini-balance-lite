...
<script>
const $=id=>document.getElementById(id);let token=localStorage.getItem('admin_token')||'';let keyRows=[];
...
const addProxy=async()=>{try{const data=await api('/admin/api/proxy-keys',{method:'POST',body:JSON.stringify({key:$('proxyKey').value,name:$('proxyName').value})});$('proxyKey').value='';$('proxyName').value='';await copyText(data.proxyKey.key);setNotice('代理 Key 已签发并复制：'+data.proxyKey.maskedKey);await load()}catch(e){setNotice(e.message)}};
...
const copyText=async(text,msg='已复制 Key')=>{try{await navigator.clipboard.writeText(text);setNotice(msg)}catch(e){const input=document.createElement('textarea');input.value=text;document.body.appendChild(input);input.select();document.execCommand('copy');input.remove();setNotice(msg)}};
const findKey=id=>keyRows.find(x=>x.id===id);
const copyKey=id=>{const item=findKey(id);if(!item?.key)return setNotice('找不到真实 Gemini Key');return copyText(item.key,'已复制真实 Gemini Key')};
const copyKeys=availableOnly=>{const rows=keyRows.filter(x=>!availableOnly||(x.status==='available'&&!x.disabled));if(!rows.length)return setNotice(availableOnly?'没有 available 真实 Gemini Key':'没有真实 Gemini Key');return copyText(rows.map(x=>x.key).join('\\n'),availableOnly?'已复制 '+rows.length+' 个 available 真实 Gemini Key':'已复制 '+rows.length+' 个真实 Gemini Key')};
const selectedKeys=()=>[...document.querySelectorAll('.key-check:checked')].map(x=>x.value);
const batchTest=async()=>{const ids=keyRows.map(x=>x.id);if(!ids.length)return setNotice('没有可检测的真实 Gemini Key');setNotice('开始批量检测 '+ids.length+' 个 Key...');let ok=0;let fail=0;for(const id of ids){try{await api('/admin/api/keys/'+id+'/test',{method:'POST'});ok++}catch(e){fail++}setNotice('批量检测中：成功 '+ok+' / 失败 '+fail+' / 总数 '+ids.length)}await load();setNotice('批量检测完成：成功 '+ok+' / 失败 '+fail+' / 总数 '+ids.length)};
const batchDelete=async()=>{const ids=selectedKeys();if(!ids.length)return setNotice('请先勾选要删除的真实 Gemini Key');if(!confirm('确定删除选中的 '+ids.length+' 个真实 Gemini Key？'))return;let ok=0;let fail=0;for(const id of ids){try{await api('/admin/api/keys/'+id,{method:'DELETE'});ok++}catch(e){fail++}}await load();setNotice('批量删除完成：成功 '+ok+' / 失败 '+fail)};
document.addEventListener('click',e=>{const b=e.target.closest('button[data-action]');if(!b)return;const id=b.dataset.id;const disabled=b.dataset.disabled==='true';const actions={'copy-key':()=>copyKey(id),'test-key':()=>testKey(id),'toggle-key':()=>toggleKey(id,disabled),'reset-key':()=>resetKey(id),'delete-key':()=>deleteKey(id),'toggle-proxy':()=>toggleProxy(id,disabled),'delete-proxy':()=>deleteProxy(id),'copy-proxy':()=>copyText(b.dataset.key,'已复制代理 Key')};actions[b.dataset.action]?.()});
$('loginBtn').onclick=login;$('loginToken').onkeydown=e=>{if(e.key==='Enter')login()};$('addBtn').onclick=add;$('addProxyBtn').onclick=addProxy;$('refreshBtn').onclick=load;$('logoutBtn').onclick=logout;$('copyAllKeysBtn').onclick=()=>copyKeys(false);$('copyAvailableKeysBtn').onclick=()=>copyKeys(true);$('batchTestBtn').onclick=batchTest;$('batchDeleteBtn').onclick=batchDelete;$('selectAllKeys').onchange=e=>document.querySelectorAll('.key-check').forEach(x=>x.checked=e.target.checked);if(token){load()}else{showLogin()}
</script>
</body>
</html>`;
