...
<main id="appView" class="hidden">
  ...
  <div class="toolbar"><h2>真实 Gemini Key 池</h2><div class="actions"><button id="copyAllKeysBtn">复制全部真实 Key</button><button class="secondary" id="copyAvailableKeysBtn">复制 available 真实 Key</button><button id="batchTestBtn">一键检测全部</button><button class="danger" id="batchDeleteBtn">删除选中</button></div><div class="small" id="summary">-</div></div>
  <section class="card"><table><thead><tr><th><input id="selectAllKeys" type="checkbox"></th><th>Key</th><th>状态</th><th>统计</th><th>最近信息</th><th>操作</th></tr></thead><tbody id="tbody"></tbody></table></section>
</main>
<script>
...
let keyRows=[];
let toastTimer=0;
const statusClass=type=>type==='success'?' success':type==='error'?' error':'';
const showToast=(text,type='info',hold=false)=>{const toast=$('toast');if(!toast)return;toast.textContent=text;toast.className='toast'+statusClass(type);if(toastTimer)clearTimeout(toastTimer);if(!hold)toastTimer=setTimeout(()=>toast.classList.add('hidden'),2600)};
const setNotice=(text,type='info',hold=false)=>{const notice=$('notice');if(notice){notice.textContent=text;notice.className='notice'+statusClass(type)}showToast(text,type,hold)};
const setLoginNotice=(text,type='info',hold=false)=>{const notice=$('loginNotice');if(notice)notice.textContent=text;showToast(text,type,hold)};
const showLogin=(msg,type='info')=>{$('loginView').classList.remove('hidden');$('appView').classList.add('hidden');$('loginToken').value=token;if(msg)setLoginNotice(msg,type)};
...
const setButtonBusy=(button,text)=>{if(!button)return()=>{};const original=button.dataset.originalText||button.textContent;button.dataset.originalText=original;button.disabled=true;button.textContent=text;return finalText=>{button.disabled=false;if(finalText){button.textContent=finalText;setTimeout(()=>{button.textContent=original},900)}else{button.textContent=original}}};
const runButton=async(button,loadingText,action)=>{const finish=setButtonBusy(button,loadingText);try{const result=await action();if(result===false){finish();return}finish('完成')}catch(e){finish('失败');setNotice(e.message||'操作失败','error')}};
const load=async(options={})=>{const quiet=!!options.quiet;try{if(!quiet)setNotice('正在刷新状态...','info',true);const data=await api('/admin/api/keys');showApp();const rows=data.keys||[];keyRows=rows;const proxies=data.proxyKeys||[];$('selectAllKeys').checked=false;$('summary').textContent='共 '+rows.length+' 个，available '+rows.filter(x=>x.status==='available'&&!x.disabled).length+' 个';$('proxySummary').textContent='共 '+proxies.length+' 个，启用 '+proxies.filter(x=>!x.disabled).length+' 个';$('proxyTbody').innerHTML=proxies.map(k=>'<tr><td><div class="mono">'+k.maskedKey+'</div><div class="small">'+(k.name||k.id)+'</div></td><td>'+(k.disabled?badge('disabled'):badge('available'))+'</td><td>请求 '+k.requestCount+'</td><td><div class="small">'+(k.lastUsedAt||'-')+'</div></td><td><div class="row-actions"><button data-action="copy-proxy" data-key="'+k.key+'">复制</button><button class="secondary" data-action="toggle-proxy" data-id="'+k.id+'" data-disabled="'+(!k.disabled)+'">'+(k.disabled?'启用':'禁用')+'</button><button class="danger" data-action="delete-proxy" data-id="'+k.id+'">删除</button></div></td></tr>').join('')||'<tr><td colspan="5">暂无代理 Key</td></tr>';$('tbody').innerHTML=rows.map(k=>'<tr><td><input class="key-check" type="checkbox" value="'+k.id+'"></td><td><div class="mono">'+k.maskedKey+'</div><div class="small">'+(k.name||k.id)+'</div></td><td>'+(k.disabled?badge('disabled'):badge(k.status))+'</td><td><div>请求 '+k.requestCount+' / 成功 '+k.successCount+' / 失败 '+k.failCount+'</div><div class="small">HTTP '+(k.lastStatus||'-')+'</div></td><td><div class="small">last: '+(k.lastUsedAt||'-')+'</div><div class="small">429: '+(k.last429At||'-')+'</div><div class="small">'+(k.lastError||'')+'</div></td><td><div class="row-actions"><button data-action="copy-key" data-id="'+k.id+'">复制</button><button data-action="test-key" data-id="'+k.id+'">检测</button><button class="secondary" data-action="toggle-key" data-id="'+k.id+'" data-disabled="'+(!k.disabled)+'">'+(k.disabled?'启用':'禁用')+'</button><button class="secondary" data-action="reset-key" data-id="'+k.id+'">重置</button><button class="danger" data-action="delete-key" data-id="'+k.id+'">删除</button></div></td></tr>').join('')||'<tr><td colspan="6">暂无真实 Gemini Key</td></tr>';if(!quiet)setNotice('状态已刷新','success');return true}catch(e){if(e.status===401){localStorage.removeItem('admin_token');showLogin('ADMIN_TOKEN 不正确或 Cloudflare 未设置 ADMIN_TOKEN','error');return false}showApp();setNotice(e.message,'error');return false}};
const login=async()=>{token=$('loginToken').value.trim();if(!token){setLoginNotice('请输入 ADMIN_TOKEN','error');return false}localStorage.setItem('admin_token',token);setLoginNotice('正在登录...','info',true);return load()};
const logout=()=>{token='';localStorage.removeItem('admin_token');showLogin('已退出登录','success');return true};
const add=async()=>{if(!$('keys').value.trim()){setNotice('请先填写真实 Gemini API Key','error');return false}setNotice('正在添加真实 Key...','info',true);await api('/admin/api/keys',{method:'POST',body:JSON.stringify({keys:$('keys').value,name:$('name').value})});$('keys').value='';await load({quiet:true});setNotice('真实 Key 已添加','success');return true};
const addProxy=async()=>{setNotice('正在签发代理 Key...','info',true);const data=await api('/admin/api/proxy-keys',{method:'POST',body:JSON.stringify({key:$('proxyKey').value,name:$('proxyName').value})});$('proxyKey').value='';$('proxyName').value='';const copied=await copyText(data.proxyKey.key,'代理 Key 已签发并复制：'+data.proxyKey.maskedKey);await load({quiet:true});if(!copied)return false;setNotice('代理 Key 已签发并复制：'+data.proxyKey.maskedKey,'success');return true};
const testKey=async id=>{setNotice('正在检测真实 Gemini Key...','info',true);const data=await api('/admin/api/keys/'+id+'/test',{method:'POST'});await load({quiet:true});setNotice('检测完成：HTTP '+data.status+(data.ok?'，可用':'，不可用'),data.ok?'success':'error');return true};
const toggleKey=async(id,disabled)=>{setNotice(disabled?'正在禁用真实 Key...':'正在启用真实 Key...','info',true);await api('/admin/api/keys/'+id,{method:'PATCH',body:JSON.stringify({disabled})});await load({quiet:true});setNotice(disabled?'真实 Key 已禁用':'真实 Key 已启用','success');return true};
const resetKey=async id=>{setNotice('正在重置真实 Key...','info',true);await api('/admin/api/keys/'+id+'/reset',{method:'POST'});await load({quiet:true});setNotice('真实 Key 已重置为 available','success');return true};
const deleteKey=async id=>{if(!confirm('确定删除这个真实 Key？'))return false;setNotice('正在删除真实 Key...','info',true);await api('/admin/api/keys/'+id,{method:'DELETE'});await load({quiet:true});setNotice('真实 Key 已删除','success');return true};
const toggleProxy=async(id,disabled)=>{setNotice(disabled?'正在禁用代理 Key...':'正在启用代理 Key...','info',true);await api('/admin/api/proxy-keys/'+id,{method:'PATCH',body:JSON.stringify({disabled})});await load({quiet:true});setNotice(disabled?'代理 Key 已禁用':'代理 Key 已启用','success');return true};
const deleteProxy=async id=>{if(!confirm('确定删除这个代理 Key？'))return false;setNotice('正在删除代理 Key...','info',true);await api('/admin/api/proxy-keys/'+id,{method:'DELETE'});await load({quiet:true});setNotice('代理 Key 已删除','success');return true};
const copyText=async(text,msg='已复制 Key')=>{if(!text){setNotice('没有可复制内容','error');return false}try{if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(text)}else{throw new Error('Clipboard unavailable')}setNotice(msg,'success');return true}catch(e){let input;try{input=document.createElement('textarea');input.value=text;input.style.position='fixed';input.style.opacity='0';document.body.appendChild(input);input.focus();input.select();const ok=document.execCommand('copy');input.remove();input=null;if(!ok)throw new Error('Copy command failed');setNotice(msg,'success');return true}catch(err){if(input)input.remove();setNotice('复制失败，请检查浏览器剪贴板权限','error');return false}}};
const findKey=id=>keyRows.find(x=>x.id===id);
const copyKey=id=>{const item=findKey(id);if(!item?.key){setNotice('找不到真实 Gemini Key','error');return false}return copyText(item.key,'已复制真实 Gemini Key')};
const copyKeys=availableOnly=>{const rows=keyRows.filter(x=>!availableOnly||(x.status==='available'&&!x.disabled));const keys=rows.map(x=>x.key).filter(Boolean);if(!keys.length){setNotice(availableOnly?'没有 available 真实 Gemini Key':'没有真实 Gemini Key','error');return false}return copyText(keys.join('\\n'),availableOnly?'已复制 '+keys.length+' 个 available 真实 Gemini Key':'已复制 '+keys.length+' 个真实 Gemini Key')};
const selectedKeys=()=>[...document.querySelectorAll('.key-check:checked')].map(x=>x.value);
const batchTest=async()=>{const ids=keyRows.map(x=>x.id);if(!ids.length){setNotice('没有可检测的真实 Gemini Key','error');return false}setNotice('开始批量检测 '+ids.length+' 个 Key...','info',true);let ok=0;let fail=0;for(const id of ids){try{const result=await api('/admin/api/keys/'+id+'/test',{method:'POST'});if(result.ok){ok++}else{fail++}}catch(e){fail++}setNotice('批量检测中：可用 '+ok+' / 不可用 '+fail+' / 总数 '+ids.length,'info',true)}await load({quiet:true});setNotice('批量检测完成：可用 '+ok+' / 不可用 '+fail+' / 总数 '+ids.length,fail?'error':'success');return true};
const batchDelete=async()=>{const ids=selectedKeys();if(!ids.length){setNotice('请先勾选要删除的真实 Gemini Key','error');return false}if(!confirm('确定删除选中的 '+ids.length+' 个真实 Gemini Key？'))return false;setNotice('正在删除选中的 '+ids.length+' 个真实 Gemini Key...','info',true);let ok=0;let fail=0;for(const id of ids){try{await api('/admin/api/keys/'+id,{method:'DELETE'});ok++}catch(e){fail++}setNotice('批量删除中：成功 '+ok+' / 失败 '+fail+' / 总数 '+ids.length,'info',true)}await load({quiet:true});setNotice('批量删除完成：成功 '+ok+' / 失败 '+fail,fail?'error':'success');return true};
document.addEventListener('click',e=>{const b=e.target.closest('button[data-action]');if(!b)return;const id=b.dataset.id;const disabled=b.dataset.disabled==='true';const actions={'copy-key':['复制中...',()=>copyKey(id)],'test-key':['检测中...',()=>testKey(id)],'toggle-key':[disabled?'禁用中...':'启用中...',()=>toggleKey(id,disabled)],'reset-key':['重置中...',()=>resetKey(id)],'delete-key':['删除中...',()=>deleteKey(id)],'toggle-proxy':[disabled?'禁用中...':'启用中...',()=>toggleProxy(id,disabled)],'delete-proxy':['删除中...',()=>deleteProxy(id)],'copy-proxy':['复制中...',()=>copyText(b.dataset.key,'已复制代理 Key')]};const action=actions[b.dataset.action];if(action)runButton(b,action[0],action[1])});
const bindButton=(id,loadingText,action)=>{$(id).onclick=e=>runButton(e.currentTarget,loadingText,action)};
bindButton('loginBtn','登录中...',login);$('loginToken').onkeydown=e=>{if(e.key==='Enter')$('loginBtn').click()};bindButton('addBtn','添加中...',add);bindButton('addProxyBtn','签发中...',addProxy);bindButton('refreshBtn','刷新中...',()=>load());bindButton('logoutBtn','退出中...',logout);bindButton('copyAllKeysBtn','复制中...',()=>copyKeys(false));bindButton('copyAvailableKeysBtn','复制中...',()=>copyKeys(true));bindButton('batchTestBtn','检测中...',batchTest);bindButton('batchDeleteBtn','删除中...',batchDelete);$('selectAllKeys').onchange=e=>document.querySelectorAll('.key-check').forEach(x=>x.checked=e.target.checked);if(token){load()}else{showLogin()}
</script>
</body>
</html>`;
