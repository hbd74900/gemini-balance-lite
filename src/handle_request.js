...
<main id="appView" class="hidden">
  ...
  <div class="toolbar"><h2>真实 Gemini Key 池</h2><div class="actions"><button id="batchTestBtn">一键检测全部</button><button class="danger" id="batchDeleteBtn">删除选中</button></div><div class="small" id="summary">-</div></div>
...
const copyText=async text=>{try{await navigator.clipboard.writeText(text);setNotice('已复制代理 Key')}catch(e){const input=document.createElement('input');input.value=text;document.body.appendChild(input);input.select();document.execCommand('copy');input.remove();setNotice('已复制代理 Key')}};
...
document.addEventListener('click',e=>{const b=e.target.closest('button[data-action]');if(!b)return;const id=b.dataset.id;const disabled=b.dataset.disabled==='true';const actions={'test-key':()=>testKey(id),'toggle-key':()=>toggleKey(id,disabled),'reset-key':()=>resetKey(id),'delete-key':()=>deleteKey(id),'toggle-proxy':()=>toggleProxy(id,disabled),'delete-proxy':()=>deleteProxy(id),'copy-proxy':()=>copyText(b.dataset.key)};actions[b.dataset.action]?.()});
$('loginBtn').onclick=login;$('loginToken').onkeydown=e=>{if(e.key==='Enter')login()};$('addBtn').onclick=add;$('addProxyBtn').onclick=addProxy;$('refreshBtn').onclick=load;$('logoutBtn').onclick=logout;$('batchTestBtn').onclick=batchTest;$('batchDeleteBtn').onclick=batchDelete;$('selectAllKeys').onchange=e=>document.querySelectorAll('.key-check').forEach(x=>x.checked=e.target.checked);if(token){load()}else{showLogin()}
</script>
</body>
</html>`;
